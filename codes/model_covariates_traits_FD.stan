...
data {
  ...
  matrix[P,K] T0_sc;                        // pre-logging value of traits, scaled
  matrix[P,K] FD_sc;                        // pre-logging value of functional diversity, scaled
...
parameters {
  ...
  real<lower=-1,upper=1> gamma [2,K]; // trait and FD effect
...
model{
  ...
  matrix[P,K] trait_effect;
  matrix[P,K] fdiv_effect;
  
  covar_effect = Covar*lambda; 
  
  for (p in 1:P){
    for (k in 1:K){
      trait_effect[p,k] = gamma[1,k]*T0_sc[p,k];
      fdiv_effect[p,k] = gamma[2,k]*FD_sc[p,k];
    }
  }
  ...
  for ( k in 1:K){
    ...
    for (p in 1:P) {
      ...
      // + covariates effet
      pred_Delta[p,k] = dist[p] * (lambda0[k] + covar_effect[p,k] + trait_effect[p,k] + fdiv_effect[p,k]);
