...
dfTraits = genusData_melt[,.(value = weighted.mean(value, agb)),.(idplot, year, trait, cohort)]

## add plot info (plot size, treatment)
dfTraits = merge(dfTraits, plotInfo)

## initial value
md_0 = dfTraits[year < 1987,.(value=mean(value)),.(idplot, trait, treat)]
...
ggplot(dfTraits, aes(x = year - 1986, y = value, group = idplot, colour = treat)) + 
  geom_line() + geom_point() + 
  geom_point(data = md_0, aes(x = 0), pch = 1, cex = 5) + 
