...
df0 = subset(dfTraits, size=="small" & year <= tlog)[,.(T0 = mean(WMT)), .(idplot, trait)]
df_pred = merge(df_pred, df0, by = c("idplot","trait"))

df_pred[, tr := T0 * ( 1 + Delta * ( t / tmax * exp( 1 - t / tmax))^theta )]
df_ci = df_pred[,.(inf = quantile(tr, 0.025), 
                   med = quantile(tr, 0.5), 
                   sup = quantile(tr, 0.975)), .(trait,idplot,t)]
df_ci = merge(df_ci, plotInfo, by = "idplot")
```

```{r}
graph_pred = function(sit){
  ggplot(subset(dfTraits, size=="small" & site == sit & treat!="ctrl"), aes(x = year - tlog, y = WMT, colour = idplot)) + 
...
df_covar = melt(df_covar, id.vars = c("site", "trait","parameter","mean","sd"), 
                measure.vars = c("prec","seas","dry","rad","BkD","Psand","CFr","Depth","CEC","loss_rel","loss_abs"), variable.name = "covariate")
...
ggplot(subset(df_covar, parameter == "Delta" & covar_typ == "dist"), aes(x = value, y = mean, size = 1/sd, weight = 1/sd)) + 
