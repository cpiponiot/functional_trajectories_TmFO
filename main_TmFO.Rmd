...
## Traits trajectory {.tabset .tabset-pills}
...
ggplot(subset(dfTraits, site=="prc"), aes(x = year - 1986, y = WMT, group = idplot, colour = treatment)) + 
...
```{r recr_traj,fig.height=10, fig.width=10}
ggplot(subset(dfTraits, site=="bsl"), aes(x = year - 1986, y = WMT, group = idplot, colour = treatment)) + 
  geom_line() + geom_point() + 
  geom_point(data = md_0, aes(x = 0, y = WMT0), pch = 1, cex = 5) + 
  ylab("MWT") + xlab("Time since logging (yr)") +
  facet_grid(trait~cohort, scales = "free")
